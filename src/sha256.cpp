...
uint32_t
SHA256::choose (uint32_t e, uint32_t f, uint32_t g)
{
    return (e & f) ^ (~e & g);
}

uint32_t
SHA256::majority (uint32_t a, uint32_t b, uint32_t c)
{
    return (a & (b | c)) | (b & c);
}
...
std::string
SHA256::toString (const uint8_t *digest)
{
    std::stringstream s;
